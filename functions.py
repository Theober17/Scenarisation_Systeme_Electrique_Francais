...
def simulateur_systeme_electrique_francais(scenario_prod, scenario_cons, ):
    ...
    min_nucbase = round((float(df_scenario["FC minimum (%)"][df_scenario["Filière"] == "nucléaire base"].iloc[0]) / 100) * Knucbase, 1)
    max_nucbase = round(Knucbase * 0.9, 1)
    ramp_nucbase = float(df_scenario["Ramping"][df_scenario["Filière"] == "nucléaire base"].iloc[0])
    ...
    min_nucflex = round((float(df_scenario["FC minimum (%)"][df_scenario["Filière"] == "nucléaire flexible"].iloc[0]) / 100) * Knucflex, 1)
    max_nucflex = round(Knucflex * 0.7, 1)
    ramp_nucflex = float(df_scenario["Ramping"][df_scenario["Filière"] == "nucléaire flexible"].iloc[0])
    ...
    min_fuel = round((float(df_scenario["FC minimum (%)"][df_scenario["Filière"] == "pétrole"].iloc[0]) / 100) * Kfuel, 2)
    max_fuel = Kfuel
    ramp_fuel = float(df_scenario["Ramping"][df_scenario["Filière"] == "pétrole"].iloc[0])
    ...
    min_gasCC = round((float(df_scenario["FC minimum (%)"][df_scenario["Filière"] == "gaz"].iloc[0]) / 100) * KgasCC, 1)
    max_gasCC = KgasCC
    ramp_gasCC = float(df_scenario["Ramping"][df_scenario["Filière"] == "gaz"].iloc[0])
    ...
    ramp_export = float(df_scenario["Ramping"][df_scenario["Filière"] == "exports"].iloc[0])
    ramp_import = float(df_scenario["Ramping"][df_scenario["Filière"] == "imports"].iloc[0])
    ...
    if Positionnemet_batteries == 1 :
        ...
        for tranche_horaire in range(8760) :
            ...
            if tranche_horaire%100==0:
                print(tranche_horaire)
    ...
    df_scenario_simule['nucléaireflexible'] = [x * (1 + perte) for x in nucflex]
